...
const HeatMap = () => {
    ...
    const renderMonthGrid = (monthIndex: number) => {
        ...
        for (let i = 0; i < 35; i++) {
            ...
            const bgColor = submissionCount > 2 ? 'bg-green-700' : submissionCount === 2 ? 'bg-green-500' : submissionCount === 1 ? 'bg-green-400' : 'bg-gray-400';
    ...
    return (
        <Card className="w-[903px] max-w-full h-50 text-blueGray-500">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                    15 submissions in the last 12 months
                </CardTitle>
                <Select defaultValue="2024">
                    <SelectTrigger className="w-[88px] h-10 bg-coolGray-900 border-gray-600 text-gray-500">
            ...
            <div className="flex w-[903px] h-11 gap-4 items-center text-sm text-blueGray-400 bg-white dark:bg-slate-800 mt-4">
            <span>Current streak: 5</span>   <span>Highest streak: 15</span>
            </div>
             </CardContent>
           </Card>
    );
