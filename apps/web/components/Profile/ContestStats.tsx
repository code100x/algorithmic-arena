...
const ContestStatItem: React.FC<{ title: string; value: string; highlight?: string }> = ({ title, value, highlight }) => (
  <div className="self-stretch h-[76px] px-4 py-3 rounded-xl flex-col justify-start items-start gap-2 flex">
    <div className="self-stretch dark:text-slate-50 text-black text-base font-medium leading-normal">{title}</div>
  ...
  return (
    <Card className="w-[439.5px] h-[336px] gap-4 font-bold border">
      <CardHeader className="items-start px-6 pt-6">
        <CardTitle className="dark:text-slate-50 text-black  text-xl font-bold">Your Contest Stats</CardTitle>
      </CardHeader>
