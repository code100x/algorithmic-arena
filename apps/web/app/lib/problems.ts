...
async function getProblemFullBoilerplateCode(problemId: string, languageId: SUPPORTED_LANGS): Promise<string> {
    return new Promise((resolve, reject) => {
        fs.readFile(path.join(__dirname, `${MOUNT_PATH}/${problemId}/boilerplate-full/function.${languageId}`), { encoding: "utf-8" }, (err, data) => {
            if (err) {
                reject(err);
            }
            resolve(data);
...
async function getProblemInputs(problemId: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
        fs.readdir(path.join(__dirname, `${MOUNT_PATH}/${problemId}/tests/inputs`), async (err, files) => {
            if (err) {
                console.log(err); 
            } else {
                await Promise.all(files.map(file => { 
                    return new Promise<string>((resolve, reject) => {
                        fs.readFile(path.join(__dirname, `${MOUNT_PATH}/${problemId}/tests/inputs/${file}`), { encoding: "utf-8" }, (err, data) => {
                            if (err) {
                                reject(err);
                            }
                            resolve(data);
...
async function getProblemOutputs(problemId: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
        fs.readdir(path.join(__dirname, `${MOUNT_PATH}/${problemId}/tests/outputs`), async (err, files) => {
            if (err) {
                console.log(err); 
            } else {
                await Promise.all(files.map(file => { 
                    return new Promise<string>((resolve, reject) => {
                        fs.readFile(path.join(__dirname, `${MOUNT_PATH}/${problemId}/tests/outputs/${file}`), { encoding: "utf-8" }, (err, data) => {
                            if (err) {
                                reject(err);
                            }
                            resolve(data);
