...
void solve()
{
    ...
    v<v<int>> prefixAnd(n, v<int>(32, 0));

    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < 32; j++)
        {
            prefixAnd[i][j] = (i == 0 ? 0 : prefixAnd[i-1][j]) + ((a[i] >> j) & 1);
        }
    }
    ...
    auto getAnd = [&](int l, int r)
    {
        ...
        for(int i = 0; i < 32; i++)
        {
            ans[i] = prefixAnd[r][i] - (l == 0 ? 0 : prefixAnd[l-1][i]);
        }
        int res = 0;
        for(int i = 0; i < 32; i++)
        {
