...
int maxx;
vi a;
void rec(int index, int sum1, int sum2, int sum3, int sum4){

    if(index == a.size()){
        if(sum1 == sum2 && sum3 == sum4){
            maxx = max(maxx, sum1 * sum3);
        }
        return;
    }

    rec(index+1, sum1 + a[index], sum2, sum3, sum4);
    rec(index+1, sum1, sum2 + a[index], sum3, sum4);
    rec(index+1, sum1, sum2, sum3 + a[index], sum4);
    rec(index+1, sum1, sum2, sum3, sum4 + a[index]);
    rec(index+1, sum1, sum2, sum3, sum4);
