...
int maxBalancedSum(vi &a){
    ...
    for(int i = 0; i < n; ++i)
    {
        // checking if i can be the eq. point
        ...
        // try to make them equal by removing something in pre
        if(pre.count(leftSum - rightSum))
        {
            ans = max(ans, rightSum);
        }

        // try to make them equal by removing something in suf
        if(suf.count(rightSum - leftSum))
        {
            ans = max(ans, leftSum);
        }
