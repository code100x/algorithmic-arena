...
void solve()
{   
    int n; cin >> n; vi query_types(n); cin >> query_types;
    cin >> n; vi values(n); cin >> values;
    ...
    for(int i = 0 ; i < n ; i++){
        if(query_types[i] == 1){
            st.insert(values[i]);
            s.insert(values[i]);
        }else if(query_types[i] == 2){
            st.erase(values[i]);
            s.erase(values[i]);
        }else if(query_types[i] == 3){
            int size = s.size();
            int median = st.getKthElement(((size + 1) / 2) - 1);
            if(size == 0) median = -1e8;
            cout << median << " ";
        }
    }
