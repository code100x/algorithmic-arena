...
async function getProblemFullBoilerplateCode(
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(
      path.join(
        __dirname,
        `../../problems/${problemId}/boilerplate-full/function.${languageId}`,
      ),
      { encoding: "utf-8" },
      (err, data) => {
        if (err) {
...
async function getProblemInputs(problemId: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    fs.readdir(
      path.join(__dirname, `../../problems/${problemId}/tests/inputs`),
      async (err, files) => {
        if (err) {
          console.log(err);
        } else {
            ...
            files.map((file) => {
              return new Promise<string>((resolve, reject) => {
                fs.readFile(
                  path.join(
                    __dirname,
                    `../../problems/${problemId}/tests/inputs/${file}`,
                  ),
                  { encoding: "utf-8" },
                  (err, data) => {
                    if (err) {
...
async function getProblemOutputs(problemId: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    fs.readdir(
      path.join(__dirname, `../../problems/${problemId}/tests/outputs`),
      async (err, files) => {
        if (err) {
          console.log(err);
        } else {
            ...
            files.map((file) => {
              return new Promise<string>((resolve, reject) => {
                fs.readFile(
                  path.join(
                    __dirname,
                    `../../problems/${problemId}/tests/outputs/${file}`,
                  ),
                  { encoding: "utf-8" },
                  (err, data) => {
                    if (err) {
